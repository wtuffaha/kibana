...
function (angular, app, $, _, kbn, moment, timeSeries, numeral) {
  ...
  var module = angular.module('kibana.panels.cardinalityhistogram', []);
  app.useModule(module);

  module.controller('cardinalityhistogram', function($scope, querySrv, dashboard, filterSrv) {
    $scope.panelMeta = {
      modals : [
        {
          description: "Inspect",
      ...
      editorTabs : [
        {
          title:'Style',
          src:'app/panels/cardinalityhistogram/styleEditor.html'
        },
        {
          title:'Queries',
          src:'app/panels/cardinalityhistogram/queriesEditor.html'
        },
      ],
      status  : "Stable",
    ...
    var _d = {
      /** @scratch /panels/cardinalityhistogram/3
       *
       * === Parameters
       * ==== Axis options
       ...
       */
      y_format    : 'none',
      /** @scratch /panels/cardinalityhistogram/5
    ...
    $scope.get_data = function(data, segment, query_id) {
      ...
      _.each(queries, function(q) {
        ...
        var dAgg = $scope.ejs.DateHistogramAggregation(q.id);
        dAgg = dAgg.field($scope.panel.time_field);
        dAgg = dAgg.interval(_interval);

        var fAgg = $scope.ejs.FilterAggregation('query_filtered');
        fAgg = fAgg.filter($scope.ejs.QueryFilter(query));
        ...
        var cAgg = $scope.ejs.CardinalityAggregation('uniq_count');
        cAgg = cAgg.field($scope.panel.value_field).precisionThreshold(1000);

        fAgg.agg(cAgg);
        dAgg.agg(fAgg);

        request = request.agg(dAgg)
        ...
        // Make sure we're still on the same query/queries
          ...
          _.each(queries, function(q) {
            ...
            _.each(query_results.buckets, function(entry) {
              ...
              var tmpVal = entry.query_filtered.uniq_count.value;
              hits += tmpVal; // The series level hits counter
              $scope.hits += tmpVal; // Entire dataset level hits counter
              counters[entry.key] = (counters[entry.key] || 0) + tmpVal;

              value = (time_series._data[entry.key] || 0) + tmpVal;
  ...
  module.directive('cardinalityhistogramChart', function(dashboard, filterSrv) {
    return {
      restrict: 'A',
      template: '<div></div>',
